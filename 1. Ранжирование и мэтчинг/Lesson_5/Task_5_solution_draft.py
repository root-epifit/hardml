...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        ...
        mu_min, mu_max  = (-1,1)
        # Это для того, чтобы получались mu как в задании:
        # для К = 7 [-0.75, -0.25, 0.25, 0.75, 1] 
        mus = np.concatenate((np.linspace(mu_min+1/(K-1), mu_max-1/(K-1), K-1),np.array([1])))
        sigmas = np.array([sigma]*(K-1)+[exact_sigma])
...
class Solution:
    ...
    def simple_preproc(self, inp_str: str) -> List[str]:
        # допишите ваш код здесь
        return nltk.word_tokenize(hadle_punctuation(inp_str).lower())
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        for df in list_of_df:
            all_texts += df['text_left'].to_list()
            all_texts += df['text_right'].to_list()
    
        all_texts_unique = list(set(all_texts))
        #all_tokens = [_ for list_of_tokens in list(map(simple_preproc, all_texts_unique)) for _ in list_of_tokens]
        all_tokens = simple_preproc(' '.join(all_texts_unique))
        frequent_tokens_dict = self._filter_rare_words(Counter(all_tokens), min_occurancies)
        return list(frequent_tokens_dict.keys())
    ...
    def _read_glove_embeddings(self, file_path: str) -> Dict[str, List[str]]:
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                word_2_vec.update({items[0]:items[1:]})
        return word_2_vec
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        vocab={'PAD':0, 'OOV':1}
        shape=(len(inner_keys)+2, len(list(embeddings.values())[0]))
    
        unk_words = list(set(inner_keys) - set(list(embeddings.keys()) ))

        matrix = np.random.uniform(low=-rand_uni_bound, high=rand_uni_bound, size=shape)
        matrix[0,:] = np.zeros(shape[1])     # PAD
        matrix[1,:] = np.ones(shape[1])      # OOV
    
        for i, word in enumerate(inner_keys):
            idx=i+2
            vocab[word]=idx
            matrix[idx,:] = np.array(embeddings.get(word, matrix[idx,:]), dtype='float')
