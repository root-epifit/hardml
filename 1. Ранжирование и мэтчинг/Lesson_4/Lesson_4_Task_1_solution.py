...
#             print(ndcgs)
    ...
    def fit(self):
        ...
        #my_plot(np.linspace(start=1, stop=self.n_estimators, num=self.n_estimators).astype(int), train_ndcgs)
        #my_plot(np.linspace(start=1, stop=self.n_estimators, num=self.n_estimators).astype(int), test_ndcgs)
...
if '__name__' == 'main':
    ...
    space = [6 + hp.randint('max_depth', 11), # 6 - 17
             hp.uniform('lr', 0.1, 0.7),
             4 + hp.randint('min_lieaf', 11)  # 4 - 15
        ]

    #укажем objective-функцию
    args_and_ndcg_list=[[0,0,0,0]]
    min_ndcg = 0

    def f(args)->Solution:
        global args_and_ndcg_list
        global min_ndcg
        x,y,z=args
    #    print(f"\n>>>>>>>> args={args} <<<<<<<<<")
        s = Solution(lr=y, max_depth=x, min_samples_leaf=z)
        s.fit()
        best_ndcg = -s.best_ndcg
    #    print(f"s.best_ndcg={s.best_ndcg}")

        if best_ndcg < min_ndcg:
            print(f"best_ndcg={-best_ndcg}, args={args}")
            min_ndcg = best_ndcg
        
        args_and_ndcg_list.append(list(args)+[best_ndcg])
        return best_ndcg
